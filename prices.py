...
<body>
  ...
  <div class="card">
    ...
    <label>Files — select multiple at once</label>
...
async function go(){
  const pw=document.getElementById('pw').value,store=document.getElementById('store').value,files=document.getElementById('fi').files;
  if(!pw||!files.length){alert('Enter password and select files');return;}
  const fd=new FormData();fd.append('password',pw);fd.append('store',store);
  for(const f of files)fd.append('files',f);
  document.getElementById('btn').disabled=true;
  document.getElementById('log').textContent=`Uploading ${files.length} file(s)...`;
  const r=await fetch('/ingest',{method:'POST',body:fd});const d=await r.json();
